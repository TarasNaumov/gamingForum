            ...
            <nav>
                <form action="{{ route('admin/subcategory/search') }}" class="mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <p><input type="radio" name="sort" value="sort_category" class="mr-2"
                                  id="category" {{ ($sort == 'sort_category')? "checked" : "" }}><label for="category">Sort
                                by category</label></p>
                        <p><input type="radio" name="sort" value="sort_title" class="mr-2"
                                  id="title" {{ ($sort == 'sort_title')? "checked" : "" }}><label for="title">Sort by
                                title</label></p>
                        <p><input type="radio" name="sort" value="sort_description" class="mr-2"
                                  id="description" {{ ($sort == 'sort_description')? "checked" : "" }}><label
                                for="description">Sort by description</label></p>
                ...
                @if(auth()->user()->status != "read_only")
                    ...
                    <form action="{{ route('admin/subcategory/create') }}" method="get">
                        @csrf
                        <button type="submit"
                                class="bg-green-600 hover:bg-green-700 p-3 transition rounded-md font-semibold text-gray-800 dark:text-gray-200 leading-tight">{{ __('CREATE SUBCATEGORY') }}</button>
                    </form>
                @endif
            </nav>
        </div>
                    ...
                    <tr class="bg-gray-800">
                        ...
                        @if(is_null($subcategory->deleted_at))
                            <td class="border p-2"><a
                                    href="{{ route('admin/subcategory/edit', $subcategory->id, $subcategory->category_id) }}"
                            ...
                            <td class="border p-2">
                                <form action="{{ route('admin/subcategory.delete', $subcategory->id) }}" method="post"
                                    ...
                                    @method('delete')
                                    <button type="submit"
                                            class="block w-full p-2 bg-red-800 hover:bg-red-900 m-0">{{ __('Delete') }}</button>
                                </form>
                            </td>
                        @else
                            <td class="border p-2" colspan="2">
                                <form action="{{ route('admin/subcategory/restore', $subcategory->id) }}" method="POST"
                                      class="w-full h-full p-0">
                                    @csrf
                                    @method('patch')
                                    <button type="submit"
