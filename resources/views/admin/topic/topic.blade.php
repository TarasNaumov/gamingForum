            ...
            <nav>
                <form action="{{ route('admin/topics/search') }}" class="mb-6">
                    ...
                    <div class="mb-4">
                        ...
                        <p><input type="radio" name="sort" value="sort_forum" class="mr-2"
                                  id="forum" {{ ($sort == 'sort_forum')? "checked" : "" }}><label for="forum">Sort by
                                forum</label></p>
                ...
                @if(auth()->user()->status != "read_only")
                    ...
                    <form action="{{ route('admin/topics/create') }}" method="get">
                        @csrf
                        <button type="submit"
                                class="bg-green-600 hover:bg-green-700 p-3 transition rounded-md font-semibold text-gray-800 dark:text-gray-200 leading-tight">{{ __('CREATE TOPIC') }}</button>
                    </form>
                @endif
            </nav>
        </div>
                ...
                @foreach($topics as $topic)
                    <tr class="bg-gray-800">
                        ...
                        @if(is_null($topic->deleted_at))
                            <td class="border p-2"><a href="{{ route('admin/topics/edit', $topic->id) }}"
                        ...
                        @else
                            <td class="border p-2" colspan="2">
                                <form action="{{ route('admin/topic/restore', $topic->id) }}" method="POST"
                                      class="w-full h-full p-0">
                                    @csrf
                                    @method('patch')
                                    <button type="submit"
